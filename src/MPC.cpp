...
SolveVector MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  bool ok = true;
  ...
  // Number of model variables (states and inputs) and constraints
  ...
  // Steering range between -25 and 25 degrees (values in radians)
  const double degree25_radians = 0.436332;
  for (size_t i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -degree25_radians;
    vars_upperbound[i] = degree25_radians;
  }
  
  // Previous latency time limits
  for (size_t i = delta_start; i < delta_start + latency_multiplier; i++) {
  ...
  // Previous throttle limits
  for (size_t i = a_start; i < a_start+latency_multiplier; i++) {
  ...
  for (auto i = 0; i < N-1 ; i++){
    ...
    solveVector.throttle.push_back(solution.x[a_start+i]);
  }
